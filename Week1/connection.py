...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['MSISDN Number']=df['MSISDN Number'].apply(lambda x:int(x))
    df['Experience Score']=df['Experience Score'].apply(lambda x: int(x))
    df['Engagement Score']=df['Engagement Score'].apply(lambda x: int(x))
    df['Satisfaction score']=df['Satisfaction score'].apply(lambda x: int (x))
...
if __name__ == "__main__":
    createDB(dbName='TelCom')
    
    createTables(dbName='TelCom')

    df = pd.read_csv(r'C:\Users\blais\Desktop\10-Academy\Week1\10-Academy\Week1\data\finalData2.csv')

    insert_to_table(dbName='TelCom', df=df, table_name='UserAnlysis')
